import topHeader
import highRiskCustomersListFetch
import pandas as pd

def highRiskCustomersListPage(st, user_name):
    ...
    highRiskCustomersListInfo = highRiskCustomersListFetch.highRiskCustomersListFetch()
    print(highRiskCustomersListInfo)
    ...
    column_names = ['Account Number', 'Account Name', 'Account Type', 'IFSC Code', 'Branch', 'Customer ID', 'Opening Balance', 'Closing Balance', 'EMI Burden %', 'User ID']
    
    customers_df = pd.DataFrame(highRiskCustomersListInfo, columns=column_names)
   
    st.subheader("🏦 High Risk Customer Bank Account Details")
    
    currency_cols = ['Opening Balance', 'Closing Balance']
