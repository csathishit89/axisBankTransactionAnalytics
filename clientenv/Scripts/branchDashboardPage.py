...
def branch_dashboard(st, user_id, user_name):
    topHeader.topHeader(st, user_name)
    branch_name = streamlit_js_eval(js_expressions="localStorage.getItem('branch_name_token')", key='branch_name_token_check')
    print(branch_name)
    managementDashboardCountInfo = managementDashboardCountFetch.managementDashboardCountFetch()
    branchesNetProfitLossInfo = branchNetProfitLoss.branchNetProfitLoss(branch_name)
    ...
    if managementDashboardCountInfo:
        ...
        if years:
            ...
            # st.selectbox("Select Branch", fetchBranches, index=0)
