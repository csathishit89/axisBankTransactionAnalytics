...
def customer_dashboard(st, user_id, user_name):
    ...
    if categoryBasedSpendInformation:
        ...
        with col1:
            st.markdown("<br>", unsafe_allow_html=True)
            st.markdown(
                f'<div class="axis-title">Category Spending Distribution {selected_year}</div>',
                unsafe_allow_html=True
            )
            
            try:
                if filtered_df["expense"].dtype == 'object':
                    filtered_df["expense"] = (
                        filtered_df["expense"]
                ...
                top10_categories = filtered_df.groupby("category")["expense"].sum().nlargest(10).index
                
                boxplot_data = filtered_df
                ...
                ax.set_xlabel("Transaction Amount (₹)", fontsize=18, fontweight='bold')
